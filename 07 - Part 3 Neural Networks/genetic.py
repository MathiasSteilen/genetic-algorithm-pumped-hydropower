...
def evaluate_fitness(individual, ps_params, spot_prices):
    ...
    return fitness_score, # return as tuple, because why make user friendly


class GA_ANN_Tournament:
    def __init__(
        self,
        plant_params,
        spot,
    ...
    def train(
        ...
        toolbox.register("attr_float", np.random.standard_normal)
        ...
        toolbox.register(
            ...
            indpb=CallbackProxy(
                lambda: np.where(
                    generation
                    <= int(config["INITIAL_EXPLORATION"] * total_generations),
